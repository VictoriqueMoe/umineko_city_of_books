...
func initServices(repos *repository.Repositories, settingsSvc settings.Service) *services {
	...
	mysterySvc := mysterysvc.NewService(repos.Mystery, repos.User, repos.AuditLog, authzSvc, blockSvc, notifSvc, settingsSvc, uploadSvc, mediaProc, hub, contentFilter)
	...
	gameRoomSvc := gameroom.NewService(repos.GameRoom, repos.User, repos.Block, notifSvc, hub, contentFilter, []gameroom.GameHandler{chess.NewHandler(), checkers.NewHandler(), othello.NewHandler()})
