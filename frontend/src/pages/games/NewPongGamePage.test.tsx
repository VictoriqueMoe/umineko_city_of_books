...
import { NewPongGamePage } from "./NewPongGamePage";
...
function renderPage() {
    return renderWithProviders(<NewPongGamePage />, { user: viewer, route: "/games/pong/new" });
}
...
describe("NewPongGamePage", () => {
    it("explains that the ball goes live after a countdown", () => {
        // given
        ...
        // then
        expect(screen.getByText(/three second countdown/)).toBeInTheDocument();
    });
    ...
    it("invites the chosen opponent to a pong game and opens the new room", async () => {
        // given
        ...
        // then
        expect(mutateAsync).toHaveBeenCalledWith({ opponentId: "user-1", gameType: "pong" });
        expect(navigate).toHaveBeenCalledWith("/games/pong/room-9");
    });
