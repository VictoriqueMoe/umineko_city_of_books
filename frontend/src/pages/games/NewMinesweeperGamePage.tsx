...
export function NewMinesweeperGamePage() {
    usePageTitle("New Minesweeper Game");
    ...
    async function handleInvite() {
        ...
        setError("");
        try {
            const room = await inviteMutation.mutateAsync({ opponentId: selected.id, gameType: "minesweeper" });
            navigate(`/games/minesweeper/${room.id}`);
        } catch (err) {
    ...
    return (
        <div className={styles.page}>
            <h2 className={styles.heading}>New Minesweeper Game</h2>
            <p>
                Pick an opponent to invite. You will both pick characters once the match starts, then race to clear the
                board.
            </p>
