...
vi.mock("../../components/games/snakesandladders/SnakesAndLaddersBoardView", () => ({
    SnakesAndLaddersBoardView: (props: BoardStubProps) => (
        <section aria-label="snakes and ladders board">
            <p>{`${props.isSpectator ? "spectating" : "playing"} ${props.room.id} as ${props.viewer?.display_name ?? "nobody"}`}</p>
            <button onClick={() => props.onRoll()}>roll the die</button>
...
function renderGame(user: UserProfile | null = host) {
    return renderWithProviders(<SnakesAndLaddersGamePage />, {
        user,
        route: "/games/snakes_and_ladders/room-1",
        path: "/games/snakes_and_ladders/:id",
    });
}

describe("SnakesAndLaddersGamePage", () => {
    it("renders nothing without a room id in the address", () => {
        // given
        stubRoom();

        // when
        const { container } = renderWithProviders(<SnakesAndLaddersGamePage />, {
            user: host,
            route: "/games/snakes_and_ladders",
            path: "/games/snakes_and_ladders",
        });
    ...
    it("reports why the room could not be opened", () => {
        // given
        stubRoom({ error: "the ladders were removed" });

        // when
        renderGame();

        // then
        expect(screen.getByText("the ladders were removed")).toBeInTheDocument();
    });
    ...
    it("shows the board and the player chat once the game is running", () => {
        ...
        // then
        expect(screen.getByRole("region", { name: "snakes and ladders board" })).toBeInTheDocument();
        expect(screen.getByText("playing room-1 as Battler")).toBeInTheDocument();
        expect(screen.getByText("player chat for room-1 with 3 watching")).toBeInTheDocument();
    });
    ...
    it("treats an outsider as a spectator with the spectator chat", () => {
        ...
        expect(screen.getByText("spectator chat for room-1 with 3 watching")).toBeInTheDocument();
    });
    ...
    it("submits a roll as the only action the player can take", async () => {
        // given
        ...
        // then
        expect(submitAction).toHaveBeenCalledWith({ type: "roll" });
    });
