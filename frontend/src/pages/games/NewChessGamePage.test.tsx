...
import { NewChessGamePage } from "./NewChessGamePage";
...
function renderPage() {
    return renderWithProviders(<NewChessGamePage />, { user: viewer, route: "/games/chess/new" });
}
...
describe("NewChessGamePage", () => {
    ...
    it("invites the chosen opponent to a chess game and opens the new room", async () => {
        // given
        ...
        // then
        expect(mutateAsync).toHaveBeenCalledWith({ opponentId: "user-1", gameType: "chess" });
        expect(navigate).toHaveBeenCalledWith("/games/chess/room-9");
    });
