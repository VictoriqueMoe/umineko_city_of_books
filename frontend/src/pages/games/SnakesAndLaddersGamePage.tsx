...
export function SnakesAndLaddersGamePage() {
    ...
    usePageTitle(
        room ? `Snakes & Ladders - ${room.players.map(p => p.display_name).join(" vs ")}` : "Snakes & Ladders",
    );
    ...
    if (room.status === "pending") {
        if (!isParticipant) {
            return (
                <div className={styles.page}>
                    <h2 className={styles.heading}>Snakes & Ladders</h2>
        ...
        return (
            <div className={styles.page}>
                ...
                {isInvitee ? (
                    <p>
                        {opponent?.display_name ?? "Someone"} has invited you to a game of snakes and ladders. Accept to
                        start - you both race to square 100.
                    </p>
                ) : (
