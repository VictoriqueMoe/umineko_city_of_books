...
vi.mock("../../components/games/checkers/CheckersBoardView", () => ({
    CheckersBoardView: (props: BoardStubProps) => (
        <section aria-label="checkers board">
            <p>{`${props.isSpectator ? "spectating" : "playing"} ${props.room.id} as ${props.viewer?.display_name ?? "nobody"}`}</p>
            <button onClick={() => props.onMove({ from: "b6", path: ["c5"] })}>step forward</button>
            <button onClick={() => props.onMove({ from: "b6", path: ["d4", "f2"] })}>chain a double jump</button>
...
function renderGame(user: UserProfile | null = host) {
    return renderWithProviders(<CheckersGamePage />, {
        user,
        route: "/games/checkers/room-1",
...
describe("CheckersGamePage", () => {
    it("renders nothing without a room id in the address", () => {
        ...
        // when
        const { container } = renderWithProviders(<CheckersGamePage />, {
            user: host,
    ...
    it("reports why the room could not be opened", () => {
        // given
        stubRoom({ error: "the board was swept away" });

        // when
        renderGame();

        // then
        expect(screen.getByText("the board was swept away")).toBeInTheDocument();
    });
    ...
    it("holds a declined invite back from the board, which never got a state", () => {
        ...
        expect(screen.queryByLabelText("checkers board")).not.toBeInTheDocument();
    });
    ...
    it("shows the board and the player chat once the game is running", () => {
        ...
        // then
        expect(screen.getByRole("region", { name: "checkers board" })).toBeInTheDocument();
        expect(screen.getByText("playing room-1 as Battler")).toBeInTheDocument();
        expect(screen.getByText("player chat for room-1 with 2 watching")).toBeInTheDocument();
    });
    ...
    it("treats an outsider as a spectator with the spectator chat", () => {
        ...
        expect(screen.getByText("spectator chat for room-1 with 2 watching")).toBeInTheDocument();
    });
    ...
    it("submits a single step as a one square path", async () => {
        // given
        ...
        // then
        expect(submitAction).toHaveBeenCalledWith({ from: "b6", path: ["c5"] });
    });

    it("keeps every hop of a chained jump in the submitted path", async () => {
        // given
        const { submitAction } = stubRoom({ room: makeRoom() });
        const user = userEvent.setup();
        renderGame(host);
        ...
        // when
        await user.click(screen.getByRole("button", { name: "chain a double jump" }));

        // then
        expect(submitAction).toHaveBeenCalledWith({ from: "b6", path: ["d4", "f2"] });
    });
