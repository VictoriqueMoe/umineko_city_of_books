...
export function MinesweeperGamePage() {
    ...
    usePageTitle(room ? `Minesweeper - ${room.players.map(p => p.display_name).join(" vs ")}` : "Minesweeper");
    ...
    if (room.status === "pending") {
        if (!isParticipant) {
            return (
                <div className={styles.page}>
                    <h2 className={styles.heading}>Minesweeper</h2>
        ...
        return (
            <div className={styles.page}>
                ...
                {isInvitee ? (
                    <p>
                        {opponent?.display_name ?? "Someone"} has invited you to a minesweeper match. Accept to start;
                        you will play simultaneously and race to clear the board.
                    </p>
                ) : (
