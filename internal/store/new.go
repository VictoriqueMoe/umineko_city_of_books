...
func New(db *sql.DB, c *cache.Manager) *repository.Repositories {
	...
	repos.Theory = repository.NewTheoryRepo(db, dao.NewTheory(db))
	...
	repos.Chat = repository.NewChatRepo(db, dao.NewChat(db))
	...
	repos.OC = repository.NewOCRepo(db, dao.NewOC(db))
