...
BEGIN
    FOR j IN SELECT id, user_id FROM journals LOOP
        ...
        LOOP
            ...
            -- Carry the comment's attached media over to the new entry so we do not
            -- lose images when the comment row is removed below (FK cascade would
            -- otherwise wipe journal_comment_media rows).
            INSERT INTO journal_entry_media (entry_id, media_url, media_type, thumbnail_url, sort_order)
            SELECT new_entry_id, media_url, media_type, thumbnail_url, sort_order
